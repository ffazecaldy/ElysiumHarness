...
- (CLI overhaul, resolved) Docs grew with the professional CLI + Swarmloop mode: README slash-command table replaced with the full set (`/help` `/model` `/key` `/connections` `/tools` `/workspace` `/status` `/history` `/clear-chat` `/save` `/swarm` `/quit`) plus streaming output, friendly provider errors, Ctrl+C semantics; new sections for Swarmloop mode (`/swarm`), provider troubleshooting (429/1113 = out of credits, plain 429 = rate limit, 401/403 = bad key, fetch failed = unreachable), and key hygiene (`.env` is gitignored, keys masked, rotate any key ever pasted into a chat). Architecture: new §7 "CLI layer" (bin/agent.ts = presentation/recovery, bin/ui.ts = zero-dep NO_COLOR-aware ANSI theme, packages/cli/src/swarm-mode.ts = Swarmloop skill runtime mapped onto the core Orchestrator); old §7/§8 renumbered to §8/§9 (no external cross-references to §7+ existed — checked).
